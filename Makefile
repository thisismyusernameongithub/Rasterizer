...
build: increment_version build\application.o build\window.o
	clang $(CFLAGS) -o build\$(APP_NAME).exe build\window.o build\application.o -L.\dependencies\lib -lgdi32 -lSDL2 -lSDL2_ttf -lm -lSDL2_image

build\application.o: src\application.c
	clang $(CFLAGS) -c src\application.c -o build\application.o $(DEFINES) -target x86_64-pc-windows-gnu

build\window.o: src\window.c
	clang $(CFLAGS) -c src\window.c -o build\window.o -mavx -target x86_64-pc-windows-gnu
